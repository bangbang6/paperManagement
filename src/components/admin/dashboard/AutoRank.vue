  ...
  <div class="sales-view-list">
    ...
    <div class="list-item-wrapper">
      <div class="list-item gray">
        ...
        <div class="list-item-name" style="width: 75px;">研究组</div>
      ...
      <div class="list-item" v-for="(item,index) in rankData" :key="item.title">
        ...
        <div class="list-item-name overflow" style="width: 270px">{{item.title}}</div>
        <div
          class="list-item-address"
          style="text-align: left;padding-left: 10px"
        >{{formatDate(item.upChainTime)}}</div>
      </div>
    </div>
  </div>
</template>
...
<script>
import { getRecentAchvlist } from '@/api/dashboard'
export default {
  data () {
    return {
      rankData: []
  ...
  mounted () {
    getRecentAchvlist().then(res => {
      console.log('achvlist', res);
      if (res.code === 200) {
